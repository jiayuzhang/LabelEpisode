 ...
 * @version  3.0.0
 */
class WC_Order_Data_Store_CPT extends Abstract_WC_Order_Data_Store_CPT implements
  ...
  protected function read_order_data(&$order, $post_object) {
    ...
    $order->set_props(
        array(
            ...
            'shipping_status' => get_post_meta($id, '_shipping_status', true),
  ...
  public function update(&$order) {
    ...
    if (!$order->get_date_paid('edit')
        ...
        && $order->has_status(apply_filters('woocommerce_payment_complete_order_status',
            $order->needs_processing() ? 'processing' : 'completed', $order->get_id(),
            $order))) {
  ...
  /**
   * Helper method that updates all the post meta for an order based on it's settings in the
   * WC_Order class.
   *
   * @param WC_Order $order Order object.
   *
  ...
  protected function update_post_meta(&$order) {
    ...
    foreach ($props_to_update as $meta_key => $prop) {
      ...
      if ('date_paid' === $prop) {
        ...
        update_post_meta($id, $meta_key,
            !is_null($value) ? $value->getTimestamp() : '');

        // In 2.6.x date_paid was stored as _paid_date in local mysql format.
        update_post_meta($id, '_paid_date',
            !is_null($value) ? $value->date('Y-m-d H:i:s') : '');

      } else if ('date_completed' === $prop) {
        // In 3.0.x we store this as a UTC timestamp.
        update_post_meta($id, $meta_key,
            !is_null($value) ? $value->getTimestamp() : '');

        // In 2.6.x date_paid was stored as _paid_date in local mysql format.
        update_post_meta($id, '_completed_date',
            !is_null($value) ? $value->date('Y-m-d H:i:s') : '');
    ...
    if (in_array('billing', $updated_props, true)
        || !metadata_exists('post', $id, '_billing_address_index')) {
      update_post_meta($id, '_billing_address_index',
          implode(' ', $order->get_address('billing')));
    }
    if (in_array('shipping', $updated_props, true)
        || !metadata_exists('post', $id, '_shipping_address_index')) {
      update_post_meta($id, '_shipping_address_index',
          implode(' ', $order->get_address('shipping')));
    }

    // If customer changed, update any downloadable permissions.
    if (in_array('customer_id', $updated_props)
        || in_array('billing_email', $updated_props)) {
      $data_store = WC_Data_Store::load('customer-download');
      $data_store->update_user_by_order_id($id, $order->get_customer_id(),
  ...
  public function get_order_id_by_order_key($order_key) {
    ...
    return $wpdb->get_var($wpdb->prepare("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE meta_key = '_order_key' AND meta_value = %s",
        $order_key));
  }
   ...
   *
   * @param string $status Order status. Function wc_get_order_statuses() returns a list of valid
   *     statuses.
   *
   * @return int
   */
  public function get_order_count($status) {
    ...
    return absint($wpdb->get_var($wpdb->prepare("SELECT COUNT( * ) FROM {$wpdb->posts} WHERE post_type = 'shop_order' AND post_status = %s",
        $status)));
  }
  ...
  public function get_orders($args = array()) {
    wc_deprecated_function('WC_Order_Data_Store_CPT::get_orders', '3.1.0',
  ...
  private function get_orders_generate_customer_meta_query($values, $relation = 'or') {
    ...
    foreach ($values as $value) {
      if (is_array($value)) {
        ...
        return new WP_Error('woocommerce_query_invalid',
            __('Invalid customer query.', 'woocommerce'), $values);
      }
    }
    ...
    if (!empty($search_fields)) {
      $order_ids = array_unique(
          array_merge(
              ...
              $wpdb->get_col(
                  $wpdb->prepare(
                      "SELECT DISTINCT p1.post_id FROM {$wpdb->postmeta} p1 WHERE p1.meta_value LIKE %s AND p1.meta_key IN ('"
                      . implode("','", array_map('esc_sql', $search_fields)) . "')",
  ...
  public function get_download_permissions_granted($order) {
    ...
    return wc_string_to_bool(get_post_meta($order_id, '_download_permissions_granted',
        true));
  }
  ...
  public function get_recorded_coupon_usage_counts($order) {
    ...
    return wc_string_to_bool(get_post_meta($order_id, '_recorded_coupon_usage_counts',
        true));
  }
  ...
  protected function get_wp_query_args($query_vars) {
    ...
    if (!empty($query_vars['post_status'])) {
      if (is_array($query_vars['post_status'])) {
        ...
        $query_vars['post_status'] = wc_is_order_status('wc-'
            . $query_vars['post_status'])
            ? 'wc-' . $query_vars['post_status'] : $query_vars['post_status'];
      }
    }
    ...
    foreach ($date_queries as $query_var_key => $db_key) {
      if (isset($query_vars[$query_var_key]) && '' !== $query_vars[$query_var_key]) {
        ...
        $wp_query_args = $this->parse_date_for_wp_query($query_vars[$query_var_key],
            $db_key, $wp_query_args);
      }
    }

    if (isset($query_vars['customer']) && '' !== $query_vars['customer']
        && array() !== $query_vars['customer']) {
      $values = is_array($query_vars['customer']) ? $query_vars['customer']
          : array($query_vars['customer']);
    ...
    return apply_filters('woocommerce_order_data_store_cpt_get_orders_query', $wp_query_args,
        $query_vars, $this);
  }
  ...
  public function query($query_vars) {
    ...
    $orders = (isset($query_vars['return']) && 'ids' === $query_vars['return'])
        ? $query->posts : array_filter(array_map('wc_get_order', $query->posts));
  ...
  public function get_order_item_type($order, $order_item_id) {
    ...
    return $wpdb->get_var($wpdb->prepare("SELECT DISTINCT order_item_type FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d and order_item_id = %d;",
        $order->get_id(), $order_item_id));
  }
}
