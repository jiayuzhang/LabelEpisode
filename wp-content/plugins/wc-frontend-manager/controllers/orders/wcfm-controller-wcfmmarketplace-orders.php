...
class WCFM_Orders_WCFMMarketplace_Controller {
  ...
  public function processing() {
    ...
    $sql = 'SELECT COUNT(commission.ID) AS count FROM ' . $wpdb->prefix
    ...
    if (!empty($_POST['search']['value'])) {
      ...
      if (!empty($_POST['filter_date_form']) && !empty($_POST['filter_date_to'])) {
        ...
        $time_filter = " AND DATE( commission.created ) BETWEEN '" . $start_date . "' AND '"
            . $end_date . "'";
        $sql .= $time_filter;
      }
    ...
    $sql = 'SELECT *, GROUP_CONCAT(item_id) order_item_ids, GROUP_CONCAT(product_id) product_id, SUM( commission.quantity ) AS order_item_count, COALESCE( SUM( commission.item_total ), 0 ) AS item_total, COALESCE( SUM( commission.item_sub_total ), 0 ) AS item_sub_total, COALESCE( SUM( commission.shipping ), 0 ) AS shipping, COALESCE( SUM( commission.tax ), 0 ) AS tax, COALESCE( SUM( commission.shipping_tax_amount ), 0 ) AS shipping_tax_amount, COALESCE( SUM( commission.total_commission ), 0 ) AS total_commission, COALESCE( SUM( commission.refunded_amount ), 0 ) AS refunded_amount, GROUP_CONCAT(is_refunded) is_refundeds, GROUP_CONCAT(refund_status) refund_statuses FROM '
        . $wpdb->prefix . 'wcfm_marketplace_orders AS commission';
    ...
    $order_sync = isset($WCFMmp->wcfmmp_marketplace_options['order_sync'])
        ? $WCFMmp->wcfmmp_marketplace_options['order_sync'] : 'no';
    ...
    if (!empty($order_summary)) {
      ...
      foreach ($order_summary as $order) {
        ...
        if (apply_filters('wcfm_is_show_order_restrict_check', false, $order->order_id,
        ...
        // Status
        if ($order_sync == 'yes') {
          $wcfm_orders_json_arr[$index][] = apply_filters('wcfm_order_status_display',
              '<span class="order-status tips wcicon-status-'
              . sanitize_title($the_order->get_status()) . ' text_tip" data-tip="'
              . wc_get_order_status_name($the_order->get_status()) . '"></span>', $the_order);
        } else {
          $wcfm_orders_json_arr[$index][] = apply_filters('wcfm_order_status_display',
              '<span class="order-status tips wcicon-status-'
              . sanitize_title($order->commission_status) . ' text_tip" data-tip="'
              . $WCFMmp->wcfmmp_vendor->wcfmmp_vendor_order_status_name($order->commission_status)
              . '"></span>', $the_order);
        }
        ...
        if (apply_filters('wcfm_allow_view_customer_name', true)) {
          ...
          if (!empty($user_info)) {
            ...
            if ($user_info->first_name || $user_info->last_name) {
              $username .= esc_html(sprintf(_x('%1$s %2$s', 'full name', 'wc-frontend-manager'),
            ...
            if ($the_order->get_billing_first_name() || $the_order->get_billing_last_name()) {
              $username = trim(sprintf(_x('%1$s %2$s', 'full name', 'wc-frontend-manager'),
                  $the_order->get_billing_first_name(), $the_order->get_billing_last_name()));
        ...
        if ($can_view_orders)
          $wcfm_orders_json_arr[$index][] = apply_filters('wcfmmp_order_label_display',
              apply_filters('wcfm_order_label_display',
                  '<a href="' . get_wcfm_view_order_url($the_order->get_id(), $the_order)
                  . '" class="wcfm_order_title">#' . esc_attr($the_order->get_order_number())
                  . '</a>' . ' ' . __('by', 'wc-frontend-manager') . ' ' . $username,
                  $the_order->get_id(), $order->product_id, $order, $username),
              $the_order->get_id());
        else
          $wcfm_orders_json_arr[$index][] = apply_filters('wcfmmp_order_label_display',
              apply_filters('wcfm_order_label_display',
                  '<span class="wcfm_order_title">#' . esc_attr($the_order->get_order_number())
                  . '</span> ' . __('by', 'wc-frontend-manager') . ' ' . $username,
                  $the_order->get_id(), $order->product_id, $order, $username),
              $the_order->get_id());
        ...
        try {
          foreach ($order_item_ids as $order_item_id) {
            ...
            $order_item_details .= '<div class=""><span class="qty">' . $line_item->get_quantity()
                . 'x</span><span class="name">' . $line_item->get_name();
            if ($product && $product->get_sku()) {
              $order_item_details .= ' (' . __('SKU:', 'wc-frontend-manager') . ' '
                  . esc_html($product->get_sku()) . ')';
            }
            if (!empty($item_meta_html)
                && apply_filters('wcfm_is_allow_order_list_item_meta', false))
              $order_item_details .= '<br />(' . $item_meta_html . ')';
        ...
        $wcfm_orders_json_arr[$index][] = '<a href="#" class="show_order_items">'
            . sprintf(_n('%d item', '%d items', $order->order_item_count, 'wc-frontend-manager'),
                $order->order_item_count) . '</a>' . $order_item_details;
        ...
        if (apply_filters('wcfm_allow_customer_billing_details', true)) {
          if ($the_order->get_formatted_billing_address()) {
            $billing_address = wp_kses($the_order->get_formatted_billing_address(),
                array('br' => array()));
          }
        }
        $wcfm_orders_json_arr[$index][] = "<div style='text-align:left;float:left'>"
            . $billing_address . "</div>";
        ...
        if (!wc_ship_to_billing_address_only() && $the_order->needs_shipping_address()
          ...
          if ($the_order->get_formatted_shipping_address()) {
            $shipping_address = wp_kses($the_order->get_formatted_shipping_address(),
                array('br' => array()));
          }
        }
        $wcfm_orders_json_arr[$index][] = "<div style='text-align:left;float:left'>"
        ...
        if ($this->is_vendor_get_shipping) {
          $gross_sales += (float)apply_filters('wcfmmmp_gross_sales_shipping_cost',
              $order->shipping, $order->vendor_id);
          if ($this->is_vendor_get_tax) {
        ...
        if ($order->is_partially_refunded || in_array(1, $is_refundeds)) {
          ...
          $gross_sales = '<del>' . wc_price($gross_sales, array('currency' => $order_currency))
              . '</del>';
          $gross_sales .= "<br/>" . wc_price($refunded_gross_sales,
                  array('currency' => $order_currency));
        } else if ($order->is_refunded) {
          $gross_sales = '<del>' . wc_price($gross_sales, array('currency' => $order_currency))
              . '</del>';
        ...
        if ($the_order->get_payment_method_title()) {
          $gross_sales .= '<br /><small class="meta">' . __('Via', 'wc-frontend-manager') . ' '
              . esc_html($the_order->get_payment_method_title()) . '</small>';
        }
        ...
        if ('pending' === $order->withdraw_status) {
          $status = '<span class="wcpv-unpaid-status">' . esc_html__('UNPAID',
                  'wc-frontend-manager') . '</span>';
        }

        if ('completed' === $order->withdraw_status) {
          $status = '<span class="wcpv-paid-status">' . esc_html__('PAID', 'wc-frontend-manager')
              . '</span>';
        }

        if ('requested' === $order->withdraw_status) {
          $status = '<span class="wcpv-pending-status">' . esc_html__('REQUESTED',
                  'wc-frontend-manager') . '</span>';
        }

        if ('cancelled' === $order->withdraw_status) {
          $status = '<span class="wcpv-void-status">' . esc_html__('CANCELLED',
                  'wc-frontend-manager') . '</span>';
        }

        if ($order->is_refunded && !in_array(0, $is_refundeds)) {
          ...
          $wcfm_orders_json_arr[$index][] = apply_filters('wcfm_vendor_order_total',
              wc_price($total, array('currency' => $order_currency)) . '<br />' . $status,
              $order->order_id, $order->product_id, $gross_sales_amt, $total, $status,
              $order_currency);
          $wcfm_orders_json_arr[$index][] = $total;
        }

        // Additional Info
        $wcfm_orders_json_arr[$index][] = apply_filters('wcfm_orders_additonal_data', '&ndash;',
            $the_order->get_id());

        // Custom Column Support Before
        $wcfm_orders_json_arr = apply_filters('wcfm_orders_custom_columns_data_before',
            $wcfm_orders_json_arr, $index, $order->ID, $order, $the_order);

        // Date
        $order_date = (version_compare(WC_VERSION, '2.7', '<')) ? $the_order->order_date
            : $the_order->get_date_created();
        $wcfm_orders_json_arr[$index][] = apply_filters('wcfm_order_date_display',
            date_i18n(wc_date_format(), strtotime($order_date)), $order->order_id, $order);

        // Action
        $actions = '';
        if (apply_filters('wcfm_is_allow_order_status_update', true)
            && !in_array('requested', $refund_statuses)
            && in_array(0, $is_refundeds)) {
          if ($order_sync == 'yes') {
        ...
        if ($can_view_orders)
          $actions .= '<a class="wcfm-action-icon" href="'
              . get_wcfm_view_order_url($the_order->get_id(), $the_order)
              . '"><span class="wcfmfa fa-eye text_tip" data-tip="' . esc_attr__('View Details',
                  'wc-frontend-manager') . '"></span></a>';


        if (!WCFM_Dependencies::wcfmu_plugin_active_check()) {
          if ($is_wcfmu_inactive_notice_show = apply_filters('is_wcfmu_inactive_notice_show',
              true)) {
            $actions .= '<a class="wcfm_wcvendors_order_mark_shipped_dummy wcfm-action-icon" href="#" data-orderid="'
                . $order->order_id . '"><span class="wcfmfa fa-truck text_tip" data-tip="'
                . esc_attr__('Mark Shipped', 'wc-frontend-manager') . '"></span></a>';
          }
        }

        $actions = apply_filters('wcfm_orders_module_actions', $actions, $order->order_id,
            $the_order, $this->vendor_id);

        $wcfm_orders_json_arr[$index][] = apply_filters('wcfmmarketplace_orders_actions', $actions,
            $user_id, $order, $the_order, $this->vendor_id);

        $index++;
      }
    }
    if (!empty($wcfm_orders_json_arr))
      $wcfm_orders_json .= json_encode($wcfm_orders_json_arr);
