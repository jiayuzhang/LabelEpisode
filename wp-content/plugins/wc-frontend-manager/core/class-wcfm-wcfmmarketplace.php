...
class WCFM_Marketplace {
  ...
  function wcfmmp_order_total_commission( $order_id ) {
    ...
    $calculated_total += ( float ) apply_filters( 'wcfmmmp_gross_sales_shipping_cost', $order_due[0]->shipping, $this->vendor_id );
    ...
    $discount_total = $order_due[0]->discount_amount;
		...
		<?php if( !$admin_fee_mode ) { ?>
			<tr>
				...
				<td class="total" style="text-align:center; <?php echo $td_style; ?>">
					<div class="view"><?php echo wc_price( $order_due[0]->item_sub_total, array( 'currency' => $order_currency ) ); ?></div>
				</td>
			</tr>
		<?php } ?>
		<?php if ( $get_tax ) { ?>
		...
		<?php if ( $get_shipping && $order->needs_shipping_address() ) { ?>
			<tr>
				...
				<td class="total" style="text-align:center; <?php echo $td_style; ?>">
					<div class="view"><?php echo wc_price( apply_filters( 'wcfmmmp_gross_sales_shipping_cost', $order_due[0]->shipping, $this->vendor_id ), array( 'currency' => $order_currency ) ); ?></div>
				</td>
			</tr>
			<?php if( $get_tax ) { ?>
				<tr>
