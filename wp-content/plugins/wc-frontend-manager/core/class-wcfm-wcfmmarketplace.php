...
class WCFM_Marketplace {
  ...
  public function __construct() {
    ...
    add_action('after_wcfm_products_manage_meta_save',
        array(&$this, 'wcfmmp_product_manage_vendor_association'), 10, 2);
    add_action('gmw_location_form_default_location',
        array(&$this, 'wcfmmp_geo_locator_default_address'), 10, 2);

    // Manage Vendor Product Export Permissions - 2.4.2
    add_filter("woocommerce_product_export_product_query_args",
        array(&$this, 'wcfmmp_product_export_query_args'), 100);
    add_filter('woocommerce_product_export_row_data',
        array(&$this, 'wcfmmp_product_export_row_data'), 100, 2);
    ...
    add_filter('wcfm_allow_order_customer_details',
        array(&$this, 'wcfmmp_is_allow_order_customer_details'));
    add_filter('wcfm_order_details_shipping_line_item',
        array(&$this, 'wcfmmp_is_allow_order_details_shipping_line_item'));
    add_filter('wcfm_order_details_tax_line_item',
        array(&$this, 'wcfmmp_is_allow_order_details_tax_line_item'));
    add_filter('wcfm_order_details_line_total_head',
        array(&$this, 'wcfmmp_is_allow_order_details_line_total_head'));
    add_filter('wcfm_order_details_line_total',
        array(&$this, 'wcfmmp_is_allow_order_details_line_total'));
    add_filter('wcfm_order_details_tax_total',
        array(&$this, 'wcfmmp_is_allow_order_details_tax_total'));
    add_filter('wcfm_order_details_fee_line_item',
        array(&$this, 'wcfmmp_is_allow_order_details_fee_line_item'));
    add_filter('wcfm_order_details_refund_line_item',
        array(&$this, 'wcfmmp_is_allow_order_details_refund_line_item'));
    add_filter('wcfm_order_details_coupon_line_item',
        array(&$this, 'wcfmmp_is_allow_order_details_coupon_line_item'));
    add_filter('wcfm_order_details_total',
        array(&$this, 'wcfmmp_is_allow_wcfm_order_details_total'));
    add_filter('wcfm_order_details_shipping_total',
        array(&$this, 'wcfmmp_is_allow_wcfm_order_details_total'));
    add_filter('wcfm_order_details_refund_total',
        array(&$this, 'wcfmmp_is_allow_wcfm_order_details_total'));
    add_action('wcfm_order_details_after_line_total_head',
        array(&$this, 'wcfmmp_after_line_total_head'));
    add_action('wcfm_after_order_details_line_total', array(&$this, 'wcfmmp_after_line_total'), 10,
        2);
    add_action('wcfm_after_order_details_shipping_total',
        array(&$this, 'wcfmmp_after_shipping_total'), 10, 2);
    add_action('wcfm_after_order_details_refund_total', array(&$this, 'wcfmmp_after_refund_total'),
        10, 2);
    ...
    // Report Filter
    add_filter('wcfm_report_out_of_stock_query_from',
        array(&$this, 'wcfmmp_report_out_of_stock_query_from'), 100, 2);
    add_filter('woocommerce_reports_order_statuses',
        array(&$this, 'wcfmmp_reports_order_statuses'));
    add_filter('woocommerce_dashboard_status_widget_top_seller_query',
        array(&$this, 'wcfmmp_dashboard_status_widget_top_seller_query'), 100, 2);
  ...
  function wcfmmp_store_logo($store_logo) {
    ...
    if (!empty($gravatar_url)) {
      ...
      if ($shop_link) {
        $store_logo = '<a class="wcfm_store_logo_icon" href="' . $shop_link
            . '" target="_blank"><img src="' . $store_logo . '" alt="Store Logo" /></a>';
      }
    }
    return $store_logo;
  }
  ...
  function wcfmmp_store_name($store_name) {
    ...
    $vendor_data = get_user_meta($user_id, 'wcfmmp_profile_settings', true);
    $shop_name = wcfm_get_option('wcfm_my_store_label', __('My Store',
        'wc-frontend-manager'));  //isset( $vendor_data['store_name'] ) ? esc_attr( $vendor_data['store_name'] ) : '';
    if ($shop_name)
      $store_name = $shop_name;
    $shop_link = wcfmmp_get_store_url($user_id);
    if ($shop_name) {
      $store_name = '<a ' . $store_open_by . ' href="'
          . apply_filters('wcfmmp_vendor_shop_permalink', $shop_link) . '">' . $shop_name . '</a>';
    } else {
      $store_name = '<a ' . $store_open_by . ' href="'
          . apply_filters('wcfmmp_vendor_shop_permalink', $shop_link) . '">' . __('Shop',
              'wc-frontend-manager') . '</a>';
    }
    return $store_name;
  }
      ...
      <div class="wcfm_orders_filter_wrap wcfm_filters_wrap">
        <?php $WCFM->library->wcfm_date_range_picker_field(); ?>
          ...
          <select name="commission-status" id="commission-status" style="width: 150px;">
              ...
              <option value="requested"><?php esc_html_e('Requested',
                    'wc-frontend-manager'); ?></option>
              <option value="completed"><?php esc_html_e('Paid', 'wc-frontend-manager'); ?></option>
              <option value="cancelled"><?php esc_html_e('Cancelled',
                    'wc-frontend-manager'); ?></option>
          </select>

          <select name="shipping-status-filter" id="shipping-status-filter" style="width: 150px;">
              <option value=''>
  ...
  function wcfmmp_wcb_include_bookings() {
    ...
    $query = "SELECT ID FROM {$wpdb->posts} as posts
							...
							AND postmeta.meta_key = '_booking_product_id' AND postmeta.meta_value in ("
    ...
    if (!empty($vendor_bookings_arr))
      return $vendor_bookings_arr;
    return array(0);
  }

  // Product Vendor association on Product save
  function wcfmmp_product_manage_vendor_association($new_product_id,
    ...
    if ($product_status == 'pending') {
      $WCFM->wcfm_notification->wcfm_admin_notification_product_review($this->vendor_id,
          $new_product_id);
    } else {
      $WCFM->wcfm_notification->wcfm_admin_notification_new_product($this->vendor_id,
          $new_product_id);
    }
  }
  ...
  function wcfmmp_geo_locator_default_address($gmw_saved_location, $args) {
    ...
    if (empty($gmw_saved_location)) {
      ...
      $map_location = isset($vendor_data['store_location'])
          ? esc_attr($vendor_data['store_location']) : '';
  ...
  function wcfmmp_coupon_types($types) {
    ...
    foreach ($types as $type => $label)
      if (!in_array($type, $wcmp_coupon_types))
        unset($types[$type]);
    return $types;
  }
  ...
  function wcfmmp_valid_line_items($items, $order_id) {
    ...
    foreach ($items as $key => $value) {
      if (in_array($value->get_variation_id(), $valid_items)
          || in_array($value->get_product_id(), $valid_items)) {
        $valid[$key] = $value;
      } else if ($value->get_product_id() == 0) {
      ...
      ?>
        <td class="line_cost">
            <div class="view">
              <?php
              if ($admin_fee_mode) {
                echo wc_price(($line_total - $order_line_due[0]->line_total),
                    array('currency' => $order_currency));
              } else {
  ...
  function wcfmmp_after_shipping_total($item, $order) {
    ...
    if (!$admin_fee_mode
        && ($get_shipping = $WCFMmp->wcfmmp_vendor->is_vendor_get_shipping($this->vendor_id))) {
      ?>
        <td class="line_cost">
          <?php
          if (apply_filters('wcfmmp_is_allow_shipping_from_order', true)
              && !apply_filters('wcfmmp_is_allow_commission_on_shipping', false)) {
              ...
              if (!empty($order_taxes)) {
                if (($tax_data = $item->get_taxes()) && wc_tax_enabled()) {
                  foreach ($order_taxes as $tax_item) {
                    ...
                    $refunded = $order->get_tax_refunded_for_item($item->get_id(), $tax_item_id,
                        'shipping');
                    $shipping_commission -= (float)$refunded;
  ...
  function wcfmmp_order_total_commission($order_id) {
    ...
    $admin_fee_mode = apply_filters('wcfm_is_admin_fee_mode', false);
    $gross_sale_order = $WCFM->wcfm_vendor_support->wcfm_get_gross_sales_by_vendor($this->vendor_id,
        '', '', $order_id);
    ...
    $calculated_total += ( float )apply_filters('wcfmmmp_gross_sales_shipping_cost',
        $order_due[0]->shipping, $this->vendor_id);
    ...
    <?php if (!$admin_fee_mode) { ?>
          <tr>
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Subtotal',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view"><?php echo wc_price($order_due[0]->item_sub_total,
                        array('currency' => $order_currency)); ?></div>
              </td>
          </tr>
    <?php } ?>
    <?php if ($get_tax) { ?>
          <tr>
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Tax',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view"><?php echo wc_price($order_due[0]->tax,
                        array('currency' => $order_currency)); ?></div>
              </td>
          </tr>
    <?php } ?>
    <?php if ($get_shipping && $order->needs_shipping_address()) { ?>
          <tr>
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Shipping',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view"><?php echo wc_price(apply_filters('wcfmmmp_gross_sales_shipping_cost',
                        $order_due[0]->shipping, $this->vendor_id),
                        array('currency' => $order_currency)); ?></div>
              </td>
          </tr>
      <?php if ($get_tax) { ?>
              <tr>
                  <th class="label" colspan="2"
                      style="text-align:right; <?php echo $td_style; ?>"><?php _e('Shipping Tax',
                        'wc-frontend-manager'); ?>:
                  </th>
                  <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                      <div class="view"><?php echo wc_price($order_due[0]->shipping_tax_amount,
                            array('currency' => $order_currency)); ?></div>
                  </td>
              </tr>
      <?php } ?>
    ...
    <?php if ($refund_total) { ?>
          <tr>
              <th class="label refunded-total" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Refunded',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total refunded-total" style="text-align:center; <?php echo $td_style; ?>">
                  -<?php echo wc_price($refund_total, array('currency' => $order_currency)); ?></td>
          </tr>
    <?php } ?>
    <?php if ($discount_total
        && $WCFMmp->wcfmmp_vendor->is_vendor_deduct_discount($this->vendor_id, $order_id)) { ?>
          <tr>
              <th class="label discount-total" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Discount',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total discount-total"
                  style="text-align:center; <?php echo $td_style; ?>"><?php echo wc_price($discount_total,
                    array('currency' => $order_currency)); ?></td>
          </tr>
    <?php } ?>
    <?php if (apply_filters('wcfm_is_allow_gross_total', true)) { ?>
          <tr class="total_cost">
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Gross Total',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view">
                    <?php
                    if ($refund_total) {
                      echo "<del>" . wc_price($calculated_total,
                              array('currency' => $order_currency)) . "</del>";
                      echo "<ins>" . wc_price($gross_sale_order,
                              array('currency' => $order_currency)) . "</ins>";
                    } else {
    ...
    if (apply_filters('wcfm_is_allow_view_commission', true)) {
      if ($admin_fee_mode) {
        ?>
          <tr>
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Total Fees',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view">
                    <?php
                    echo wc_price(($gross_sale_order - $total),
                        array('currency' => $order_currency));
                    ?>
                  </div>
              </td>
        ...
        ?>
          <tr>
              <th class="label" colspan="2"
                  style="text-align:right; <?php echo $td_style; ?>"><?php _e('Total Earning',
                    'wc-frontend-manager'); ?>:
              </th>
              <td class="total" style="text-align:center; <?php echo $td_style; ?>">
                  <div class="view"><?php echo wc_price($total,
                        array('currency' => $order_currency)); ?></div>
              </td>
          </tr>
        <?php
  ...
  function wcfmmp_reports_get_order_report_data($result) {
    ...
    if (!empty($result) && is_array($result)) {
      foreach ($result as $result_key => $result_val) {
        if (!in_array($result_val->product_id, $products))
