...
class WCFM_Ajax {
  ...
  public function __construct() {
    ...
    add_action('wp_ajax_wcfm_generate_variation_attributes',
        array(&$this, 'wcfm_generate_variation_attributes'));
    ...
    add_action('wp_ajax_wcfm_order_mark_shipped', array(&$this, 'wcfm_order_mark_shipped'));
    ...
    add_action('wp_ajax_wcfm-dismiss-addon-inactive-notice',
        array($this, 'wcfm_dismiss_inactive_addon_notice'));
    ...
    add_action('wp_ajax_wcfm_json_search_products_and_variations',
        array($this, 'wcfm_json_search_products_and_variations'));

    // WCfM Ajax Product Search
    add_action('wp_ajax_wcfm_json_search_products_with_variations',
        array($this, 'wcfm_json_search_products_with_variations'));
    ...
    add_action('wp_ajax_wcfm_email_verification_code',
        array(&$this, 'wcfm_email_verification_code'));
    ...
    add_action('wp_ajax_wcfm_store_external_product_view_update',
        array(&$this, 'wcfm_store_external_product_view_update'));
    add_action('wp_ajax_nopriv_wcfm_store_external_product_view_update',
        array(&$this, 'wcfm_store_external_product_view_update'));
  }
    ...
    if (isset($_POST['controller'])) {
      ...
      switch ($controller) {
        ...
        case 'wcfm-products-manage':
          if (wcfm_is_booking()) {
            include_once($this->controllers_path
                . 'wc_bookings/wcfm-controller-wcbookings-products-manage.php');
            new WCFM_WCBookings_Products_Manage_Controller();
          }
          // Third Party Plugin Support
          include_once($this->controllers_path
              . 'products-manager/wcfm-controller-thirdparty-products-manage.php');
          new WCFM_ThirdParty_Products_Manage_Controller();

          // Custom Field Plugin Support
          include_once($this->controllers_path
              . 'products-manager/wcfm-controller-customfield-products-manage.php');
          new WCFM_Custom_Field_Products_Manage_Controller();

          include_once($this->controllers_path
              . 'products-manager/wcfm-controller-products-manage.php');
        ...
        case 'wcfm-orders':
          if ($WCFM->is_marketplace
              && (wcfm_is_vendor()
                  || apply_filters('wcfm_is_show_marketplace_orders', false))) {
            if (apply_filters('wcfm_is_show_marketplace_itemized_orders', false)) {
              include_once($this->controllers_path . 'orders/wcfm-controller-'
                  . $WCFM->is_marketplace . '-itemized-orders.php');
            } else {
              include_once($this->controllers_path . 'orders/wcfm-controller-'
                  . $WCFM->is_marketplace . '-orders.php');
            }
            if ($WCFM->is_marketplace == 'wcvendors')
              new WCFM_Orders_WCVendors_Controller();
            else if ($WCFM->is_marketplace == 'wcpvendors')
              new WCFM_Orders_WCPVendors_Controller();
            else if ($WCFM->is_marketplace == 'wcmarketplace')
              new WCFM_Orders_WCMarketplace_Controller();
            else if ($WCFM->is_marketplace == 'dokan')
              new WCFM_Orders_Dokan_Controller();
        ...
        case 'wcfm-reports-out-of-stock':
          include_once($this->controllers_path
              . 'reports/wcfm-controller-reports-out-of-stock.php');
        ...
        case 'wcfm-settings':
          if ($WCFM->is_marketplace && wcfm_is_vendor()) {
            include_once($this->controllers_path . 'settings/wcfm-controller-'
                . $WCFM->is_marketplace . '-settings.php');
            if ($WCFM->is_marketplace == 'wcvendors')
              new WCFM_Settings_WCVendors_Controller();
            else if ($WCFM->is_marketplace == 'wcpvendors')
              new WCFM_Settings_WCPVendors_Controller();
            else if ($WCFM->is_marketplace == 'wcmarketplace')
              new WCFM_Settings_WCMarketplace_Controller();
            else if ($WCFM->is_marketplace == 'dokan')
              new WCFM_Settings_Dokan_Controller();
            else if ($WCFM->is_marketplace == 'wcfmmarketplace')
        ...
        case 'wcfm-knowledgebase-manage':
          include_once($this->controllers_path
              . 'knowledgebase/wcfm-controller-knowledgebase-manage.php');
        ...
        case 'wcfm-vendors-manage-marketplace-settings':
          include_once($this->controllers_path
              . 'settings/wcfm-controller-wcfmmarketplace-settings.php');
          new WCFM_Settings_Marketplace_Controller();
          break;

        case 'wcfm-vendors-manage-marketplace-shipping-settings':
          include_once($this->controllers_path
              . 'vendors/wcfm-controller-wcfmmarketplace-shipping-settings.php');
  ...
  function wcfm_add_taxonomy_new_term() {
    ...
    if (taxonomy_exists($taxonomy)) {
      ...
      if (is_wp_error($result)) {
        ...
        $author_is_vendor = 1;
        $message_to = 0;
        $wcfm_messages = sprintf(__('A new %s <b>%s</b> added to the store by <b>%s</b>',
            'wc-frontend-manager'), ucfirst($taxonomy), $new_term,
            get_user_by('id', $author_id)->display_name);
        $WCFM->wcfm_notification->wcfm_send_direct_message($author_id, $message_to,
            $author_is_admin, $author_is_vendor, $wcfm_messages, 'new_taxonomy_term');

        // Sending front-end HTML
        $term = get_term_by('id', $result['term_id'], $taxonomy);
        echo '<li class="product_cats_checklist_item checklist_item_' . esc_attr($term->term_id)
            . '" data-item="' . esc_attr($term->term_id) . '">';
        ...
        if (($taxonomy != 'product_cat') && ($taxonomy != 'wcfm_knowledgebase_category')) {
          echo '<label class="selectit">' . $nbsp
              . '<input type="checkbox" class="wcfm-checkbox" name="product_custom_taxonomies['
              . $taxonomy . '][]" value="' . esc_attr($term->term_id) . '" checked />'
              . esc_html($term->name) . '</label>';
        } else {
          echo '<label class="selectit">' . $nbsp
              . '<input type="checkbox" class="wcfm-checkbox" name="product_cats[]" value="'
              . esc_attr($term->term_id) . '" checked /><span>' . esc_html($term->name)
              . '</span></label>';
        }
        echo '</li>';
      }
  ...
  public function wcfm_generate_variation_attributes() {
    ...
    if (isset($wcfm_products_manage_form_data['attributes'])
        && !empty($wcfm_products_manage_form_data['attributes'])) {
      $pro_attributes = '{';
      $attr_first = true;
      foreach ($wcfm_products_manage_form_data['attributes'] as $attributes) {
        if (isset($attributes['is_variation'])) {
          if (isset($attributes['is_active']) && !empty($attributes['name'])
            ...
            if ($attributes['is_taxonomy']) {
              $pro_attributes .= '"' . sanitize_title($attributes['tax_name']) . '": { "name" : " '
              ...
              if (!is_array($attributes['value'])) {
                ...
                foreach ($att_values as $att_value) {
                  ...
                  if ($is_first)
                    $is_first = false;
                  $pro_attributes .= '"' . sanitize_title($att_value) . '": "'
                      . wcfm_removeslashes(addslashes(esc_attr(trim($att_value)))) . '"';
                }
              } else {
                ...
                foreach ($att_values as $att_value) {
                  ...
                  if ($att_term) {
                    $pro_attributes .= '"' . $att_term->slug . '": "'
                        . wcfm_removeslashes(addslashes(esc_attr($att_term->name))) . '"';
                  } else {
                    $pro_attributes .= '"' . sanitize_title($att_value) . '": "'
                        . wcfm_removeslashes(addslashes(esc_attr(trim($att_value)))) . '"';
                  }
              ...
              $pro_attributes .= '"' . sanitize_title($attributes['name']) . '": { "name" : " '
                  . $attributes['name'] . ' ", "data" : {';
              $att_values = explode(WC_DELIMITER, $attributes['value']);
              $is_first = true;
              foreach ($att_values as $att_value) {
                ...
                if ($is_first)
                  $is_first = false;
                $pro_attributes .= '"' . wcfm_removeslashes(addslashes(esc_attr(trim($att_value))))
                    . '": "' . wcfm_removeslashes(addslashes(esc_attr(trim($att_value)))) . '"';
              }
              $pro_attributes .= '} }';
            }
   ...
   */
  public function wcfm_order_mark_shipped() {
    global $WCFM;

    $order_id = $_POST['orderid'];
    $shipped_to = $_POST['shipped_to'];
    $tracking = $_POST['tracking_info'];
    ...
    if ($order_id && $shipped_to) {
      ...
      $wcfm_messages = sprintf(
          __('<b>%s</b> order shipping status updated to <b>%s</b> by <b>%s</b>',
              'wc-frontend-manager'),
  ...
  public function wcfm_order_mark_complete() {
    ...
    if (wc_is_order_status('wc-completed') && $order_id) {
      ...
      $wcfm_messages = sprintf(__('<b>%s</b> order status updated to <b>%s</b> by <b>%s</b>',
          'wc-frontend-manager'), '#<a target="_blank" class="wcfm_dashboard_item_title" href="'
          . get_wcfm_view_order_url($order_id) . '">' . $order->get_order_number() . '</a>',
          wc_get_order_status_name('completed'), $shop_name);
      $is_customer_note = apply_filters('wcfm_is_allow_order_update_note_for_customer', '1');
      $comment_id = $order->add_order_note($wcfm_messages, $is_customer_note);
      if (wcfm_is_vendor()) {
        add_comment_meta($comment_id, '_vendor_id', $user_id);
  ...
  public function wcfm_modify_order_status() {
    ...
    if (wc_is_order_status($order_status) && $order_id) {
      ...
      $wcfm_messages = sprintf(__('<b>%s</b> order status updated to <b>%s</b> by <b>%s</b>',
          'wc-frontend-manager'), '#<a target="_blank" class="wcfm_dashboard_item_title" href="'
          . get_wcfm_view_order_url($order_id) . '">' . $order->get_order_number() . '</a>',
          wc_get_order_status_name(str_replace('wc-', '', $order_status)), $shop_name);
      $is_customer_note = apply_filters('wcfm_is_allow_order_update_note_for_customer', '1');
      $comment_id = $order->add_order_note($wcfm_messages, $is_customer_note);
      if (wcfm_is_vendor()) {
        add_comment_meta($comment_id, '_vendor_id', $user_id);
  ...
  function sales_by_vendor_change_url() {
    ...
    if (isset($_POST['vendor_manager_change']) && !empty($_POST['vendor_manager_change'])) {
      $vendor_id = absint($_POST['vendor_manager_change']);
      echo '{"status": true, "redirect": "' . get_wcfm_reports_url('',
              'wcfm-reports-sales-by-vendor', $vendor_id) . '"}';
    }
  ...
  function wcfm_json_search_products_and_variations() {
    ...
    if (!empty($products_objs)) {
      foreach ($products_objs as $products_obj) {
        ...
        $products_array[esc_attr($products_obj->ID)] = (!empty($product_data))
            ? wp_kses_post($product_data->get_formatted_name()) : $products_obj->ID;
      }
    }
  ...
  function wcfm_json_search_products_with_variations() {
    ...
    if (!empty($products_objs)) {
      foreach ($products_objs as $products_obj) {
        ...
        $wcfm_sv_product_types = apply_filters('wcfm_sv_product_types',
            array('simple', 'variable'));
        if (in_array($product_type, $wcfm_sv_product_types)) {
          $variations = array();

          $wcfm_variable_product_types = apply_filters('wcfm_variable_product_types',
          ...
          $products_array[esc_attr($products_obj->ID)]['label'] = (!empty($product_data))
              ? wp_kses_post($product_data->get_formatted_name()) : $products_obj->ID;
  ...
  function wcfm_json_search_taxonomies() {
    ...
    $term = wc_clean(empty($term) ? stripslashes($_REQUEST['term']) : $term);

    $taxonomy = wc_clean(empty($taxonomy) ? stripslashes($_REQUEST['taxonomy']) : 'product_cat');
    $parent = wc_clean(empty($parent) ? stripslashes($_REQUEST['parent']) : $parent);
    ...
    if (!empty($terms)) {
      foreach ($terms as $term) {
        if (apply_filters('wcfm_is_allow_hide_uncatorized', false)
  ...
  function wcfm_email_verification_code() {
    ...
    if ($user_email) {
      ...
      $verification_mail_subject = "{site_name}: " . __("Email Verification Code",
              "wc-frontend-manager") . " - " . $verification_code;
      $verification_mail_body = '<br/>' . __('Hi', 'wc-frontend-manager') .
          ',<br/><br/>' .
          sprintf(__('Here is your email verification code - <b>%s</b>', 'wc-frontend-manager'),
              '{verification_code}') .
      ...
      $message = apply_filters('wcfm_email_content_wrapper', $message,
          __('Email Verification', 'wc-multivendor-membership'));

      wp_mail($user_email, $subject, $message);

      echo '{"status": true, "message": "' . __('Email verification code send to your email.',
              'wc-frontend-manager') . '"}';
    } else {
      echo '{"status": false, "message": "'
          . __('Email verification not working right now, please try after sometime.',
              'wc-frontend-manager') . '"}';
    }
    die;
  }
  ...
  function wcfm_vendor_disable() {
    ...
    if (isset($_POST['memberid'])) {
      ...
      $wcfm_messages = sprintf(__('<b>%s</b> (Store: <b>%s</b>) has been disabled.',
          'wc-frontend-manager'), $member_user->first_name, $vendor_store);
      $WCFM->wcfm_notification->wcfm_send_direct_message(-2, 0, 1, 0, $wcfm_messages,
          'vendor-disable');

      // Vendor Notification
      $wcfm_messages = sprintf(__('Your Store: <b>%s</b> has been disabled.',
          'wc-frontend-manager'), $vendor_store);
      $WCFM->wcfm_notification->wcfm_send_direct_message(-1, $member_id, 1, 0, $wcfm_messages,
          'vendor-disable');

      echo '{"status": true, "message": "' . __('Vendor successfully disabled.',
              'wc-frontend-manager') . '"}';
      die;
    }
    echo '{"status": false, "message": "'
        . __('Vendor can not be disabled right now, please try after sometime.',
            'wc-frontend-manager') . '"}';
    die;
  }
  ...
  function wcfm_vendor_enable() {
    ...
    if (isset($_POST['memberid'])) {
      ...
      } else if ($is_marketplace == 'wcpvendors') {
        $member_user->set_role('wc_product_vendors_admin_vendor');
      } else if ($is_marketplace == 'wcvendors') {
        $member_user->set_role('vendor');
      } else if ($is_marketplace == 'dokan') {
        $member_user->set_role('seller');
      } else if ($is_marketplace == 'wcfmmarketplace') {
      ...
      $wcfm_messages = sprintf(__('<b>%s</b> (Store: <b>%s</b>) has been enabled.',
          'wc-frontend-manager'), $member_user->first_name, $vendor_store);
      $WCFM->wcfm_notification->wcfm_send_direct_message(-2, 0, 1, 0, $wcfm_messages,
          'vendor-enable');

      // Vendor Notification
      $wcfm_messages = sprintf(__('Your Store: <b>%s</b> has been enabled.', 'wc-frontend-manager'),
          $vendor_store);
      $WCFM->wcfm_notification->wcfm_send_direct_message(-1, $member_id, 1, 0, $wcfm_messages,
          'vendor-enable');


      echo '{"status": true, "message": "' . __('Vendor successfully enabled.',
              'wc-frontend-manager') . '"}';
      die;
    }
    echo '{"status": false, "message": "'
        . __('Vendor can not be enabled right now, please try after sometime.',
            'wc-frontend-manager') . '"}';
    die;
  }
  ...
  function wcfm_login_popup_submit() {
    ...
    if (empty($wcfm_login_popup_form_data['wcfm_login_popup_username'])) {
      echo '{"status": false, "message": "' . __('Please insert username before submit.',
              'wc-frontend-manager') . '"}';
      die;
    }

    if (empty($wcfm_login_popup_form_data['wcfm_login_popup_password'])) {
      echo '{"status": false, "message": "' . __('Please insert password before submit.',
              'wc-frontend-manager') . '"}';
      die;
    }

    if (!validate_username($wcfm_login_popup_form_data['wcfm_login_popup_username'])
        || !username_exists($wcfm_login_popup_form_data['wcfm_login_popup_username'])) {
      echo '{"status": false, "message": "' . __('Please insert a valid username.',
              'wc-frontend-manager') . '"}';
      die;
    }
  ...
  function wcfm_store_external_product_view_update() {
    ...
    if (isset($_POST['product_id']) && !empty($_POST['product_id'])) {
      ...
      if (!isset($_SESSION['wcfm_pages']) || !isset($_SESSION['wcfm_pages']['products'])
          || (isset($_SESSION['wcfm_pages']) && isset($_SESSION['wcfm_pages']['products'])
              && !in_array($product_id, $_SESSION['wcfm_pages']['products']))) {
        ...
        if (!$wcfm_product_views)
          $wcfm_product_views = 1;
