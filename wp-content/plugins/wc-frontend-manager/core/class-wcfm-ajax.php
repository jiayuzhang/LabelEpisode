  	...
  	$term = wc_clean( empty( $term ) ? stripslashes( $_REQUEST['term'] ) : $term );
		...
		if( !empty( $terms ) ) {
			foreach ( $terms as $term ) {
				if( apply_filters( 'wcfm_is_allow_hide_uncatorized', false ) && ( $term->slug == 'uncategorized' ) ) continue;
	...
	function wcfm_login_popup_submit() {
	  ...
	  if ( empty( $wcfm_login_popup_form_data['wcfm_login_popup_username'] ) ) {
		...
		if ( !validate_username( $wcfm_login_popup_form_data['wcfm_login_popup_username'] ) || !username_exists( $wcfm_login_popup_form_data['wcfm_login_popup_username'] ) ) {
			echo '{"status": false, "message": "' . __( 'Please insert a valid username.', 'wc-frontend-manager' ) . '"}';
			die;
		}
