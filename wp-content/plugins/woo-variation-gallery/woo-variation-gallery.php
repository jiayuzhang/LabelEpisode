        ...
        return class_exists( 'Woo_Variation_Gallery_Pro' );
			...
			public function php_requirement_notice() {
				if ( ! $this->is_required_php_version() ) {
					...
					$message = wp_kses( __( "It's required to use latest version of PHP to use <strong>WooCommerce Variation Gallery</strong>.", 'woo-variation-gallery' ), array( 'strong' => array() ) );
			...
			public function wc_requirement_notice() {
				...
				if ( ! $this->is_wc_active() ) {
					...
					$message = wp_kses( __( "<strong>WooCommerce Variation Gallery</strong> is an add-on of ", 'woo-variation-gallery' ), array( 'strong' => array() ) );
