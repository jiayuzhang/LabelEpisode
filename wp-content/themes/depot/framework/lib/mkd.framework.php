...
class DepotMikadoFramework {
    ...
    private function __construct() {
        $this->mkdOptions = DepotMikadoOptions::get_instance();
        $this->mkdMetaBoxes = DepotMikadoMetaBoxes::get_instance();
    }
	...
	public function setSkin(DepotMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class DepotMikadoSkinManager
 *
...
/**
 * Class DepotMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class DepotMikadoSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(depot_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
		...
		return MIKADO_ASSETS_ROOT.'/img/logo.ico';
	}
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo depot_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class DepotMikadoOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $depot_options)) {
            return $depot_options[$key];
...
/*
   Class: DepotMikadoAdminPage
   A class that initializes Mikado Admin Page
*/
class DepotMikadoAdminPage implements iDepotMikadoLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new DepotMikadoFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iDepotMikadoRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: DepotMikadoMetaBoxes
...
/*
   Class: DepotMikadoMetaBox
   A class that initializes Mikado Meta Box
*/
class DepotMikadoMetaBox implements iDepotMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $depot_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if (!function_exists( 'depot_mikado_init_framework_variable')) {
    function depot_mikado_init_framework_variable() {
        global $depot_Framework;

        $depot_Framework = DepotMikadoFramework::get_instance();
        $mkdSkinManager = new DepotMikadoSkinManager();
        $depot_Framework->setSkin($mkdSkinManager->getSkin());
    }

    add_action('depot_mikado_before_options_map', 'depot_mikado_init_framework_variable');
}
?>